...
!function ($) {
  ...
  Storage.prototype = {
    ...
    , get: function ( key, placeholder ) {
      return localStorage.getItem( key ) ? localStorage.getItem( key ) : 'undefined' 
		  !== typeof placeholder ? placeholder : null;
    }
  ...
  * =============================== */
  ...
  Garlic.prototype = {
    ...
    , init: function ( type, element, storage, options ) {
      ...
      this.$element.on( this.options.events.join( '.' + this.type + ' ') 
		  , false, $.proxy( this.persist, this ) );
    ...
    , persist: function () {
      ...
      if ( this.$element.is( 'input[type=checkbox]' ) ) {
        return this.storage.set( this.path , this.$element.attr( 'checked' ) ?
			'checked' : 'unchecked' );
      }
    ...
    , retrieve: function () {
      if ( this.storage.has( this.path ) ) {
        ...
        // if conflictManager enabled, manage fields with already provided data, 
        ...
        if ( this.$element.is( 'input[type=radio], input[type=checkbox]' ) ) {
          ...
          if ( 'checked' === this.storage.get( this.path ) || this.storage.get( this.path ) 
			  === this.$element.val() ) {
    ...
    , detectConflict: function() {
      ...
      if ( this.$element.val() && this.storage.get( this.path ) !== this.$element.val() ) {
        ...
        if ( this.$element.is( 'select' ) ) {
          ...
          // foreach each options except first one, always considered as selected,
		  //seeking for a default selected one
          this.$element.find( 'option' ).each( function () {
            if ( $( this ).index() !== 0 && $( this ).attr( 'selected' ) && $( this ).val() 
				!== self.storage.get( this.path ) ) {
    ...
    /* manage here the conflict, show default value depending 
	on options.garlicPriority value */
    , conflictManager: function () {

      // user can define here a custom function that could stop Garlic default behavior, 
	  //if returns false
      if ( 'function' === typeof this.options.conflictManager.onConflictDetected 
        && !this.options.conflictManager.onConflictDetected
		  ( this.$element, this.storage.get( this.path ) ) ) {
        return false;
      }
    ...
    , swap: function () {
      ...
      this.$element.data( 'swap-state', 'garlic' === this.$element.data( 'swap-state' ) ? 
		  'default' : 'garlic' );
      this.$element.data( 'swap-data', this.$element.val());
    ...
    /* retuns an unique identifier for form elements, depending on their behaviors:
       ...
       * checkbokes: domain > pathname > form.<attr.name>[:eq(x)] > [fieldset, div, span..] >
			input.<attr.name>[:eq(y)]
          cuz' they have the same name, must detect their exact position in the form. 
		  detect the exact hierarchy in DOM elements

       * other inputs: domain > pathname > form.<attr.name>[:eq(x)] >
			input.<attr.name>[:eq(y)]
    ...
    */
    , getPath: function () {
      ...
      while ( node.length ) {
        ...
        // exept for checkboxes that need exact path, cuz have same name 
		//and sometimes same eq()!
      ...
      return 'garlic:' + document.domain + ( this.options.domain ? '*' : window.location.pathname )
		  + '>' + path;
    }
  ...
  * ========================= */
  ...
  $.fn.garlic = function ( option, fn ) {
    ...
    // this plugin heavily rely on local Storage. If there is no localStorage
	//or data-storage=false, no need to go further
    if ( !storage.defined ) {
      return false;
    }
    ...
    this.each(function () {
      ...
      // add here a return instance, cuz' we could call public methods 
	  //on single elems with data[option]() above
  ...
  * ========================= */
  ...
  $.fn.garlic.defaults = {
      destroy: true                                                                              
// remove or not localstorage on submit & clear
    , inputs: 'input, textarea, select'                                                          
// Default supported inputs.
    , events: [ 'DOMAttrModified', 'textInput', 'input', 'change', 'keypress', 'paste', 'focus' ]
// events list that trigger a localStorage
    , domain: false                                                                              
// store et retrieve forms data accross all domain, not just on
    , conflictManager: {
        enabled: true                                                                             
// manage default data and persisted data. If false, persisted data will always replace default ones
      , garlicPriority: true                                                                     
 // if form have default data, garlic persisted data will be shown first 
      , template: '<span class="garlic-swap"></span>'                                             
 // template used to swap between values if conflict detected
      , message: 'This is your saved data. Click here to see default one'                        
 // default message for swapping data / state
      , onConflictDetected: function ( item, storedVal ) { return true; }                        
 // This function will be triggered if a conflict is detected on an item. Return true if you want Garlic behavior,
	//return false if you want to override it
    }
  }
